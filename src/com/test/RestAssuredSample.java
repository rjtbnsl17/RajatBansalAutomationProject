...
public class RestAssuredSample {
	...
	public Response PostRequest(String path) {
		...
		response = given().header("Content-Type","application/json").body(new File(path)).post();  // json file is passing in body
	...
	public Response queryParam() {
		...
		response = given().queryParam("q", "London,uk").queryParam("appid", "2b1fd2d7f77ccf1b7de9b441571b39b8").when().get();
	...
	public Response jsonbody(String newvalue1) {
		...
		response   = given().baseUri("https://restful-booker.herokuapp.com/booking").body(newvalue1).log().all().when().get();
